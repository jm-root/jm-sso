...
var SSO = function () {
  ...
  _createClass(SSO, [{
    ...
    value: function signout(token, cb) {
      var self = this;
      this.isSignon(token, function (err, doc) {
        if (err) return cb(err, doc);
        var id = doc.id;
        self.tokenMan.delete(token, function (err, doc) {
          if (!err && doc) self.emit('signout', { id: id });
          cb && cb(err, doc);
        });
      });
      return this;
