  ...
  async add (opts = {}) {
    ...
    try {
      ...
      if (this.enableId && opts.id) {
        ...
        let expire = await redis.ttlAsync(this.getIdKey(opts.id))
        if (expire < Number(opts.expire)) expire = opts.expire
        await redis.setAsync(this.getIdKey(opts.id), JSON.stringify(mapToken), 'EX', expire)
      }
      return opts
    } catch (e) {
      logger.error(e)
  ...
  async touch (token, opts = {}) {
    ...
    try {
      ...
      if (this.enableId && doc.id) {
        let expire = await this.redis.ttlAsync(this.getIdKey(doc.id))
        if (expire < Number(doc.expire)) expire = doc.expire
        await this.redis.expireAsync(this.getIdKey(doc.id), expire)
      }
      return doc
    } catch (e) {
      logger.error(e)
